...
async function seedSystemAdmin() {
  if (!ADMIN_EMAIL || !ADMIN_PASSWORD) {
    console.error('Error: SYSTEM_ADMIN_EMAIL and SYSTEM_ADMIN_PASSWORD environment variables are required');
    console.error('\nUsage:');
  ...
  try {
    ...
    if (!systemAdminRole) {
      ...
      systemAdminRole = await rolesCollection.findOne({ _id: roleInsertResult.insertedId });
      console.log(`✓ SYSTEM_ADMIN role created with ID: ${systemAdminRole._id}`);
    } else {
      console.log(`✓ SYSTEM_ADMIN role already exists with ID: ${systemAdminRole._id}`);
    }
    ...
    if (!user) {
      ...
      user = await usersCollection.findOne({ _id: userInsertResult.insertedId });
